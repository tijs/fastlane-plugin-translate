    ...
    class TranslateWithDeeplAction < Action
      def self.run(params)
        ...
        if unsupported_languages.any?
          UI.important("⚠️  Languages not supported by DeepL: #{unsupported_languages.map { |l| "#{Fastlane::Translate::LanguageRegistry.language_name(l)} (#{l})" }.join(', ')}")
        end
        
        if supported_languages.empty?
          UI.user_error!("❌ No DeepL-supported languages found in xcstrings file")
        end
      ...
      def self.find_xcstrings_file(provided_path)
        if provided_path
          unless File.exist?(provided_path)
            UI.user_error!("❌ Localizable.xcstrings file not found at: #{provided_path}")
          end
        ...
        if xcstrings_files.empty?
          UI.user_error!("❌ No Localizable.xcstrings files found. Please specify the path with xcstrings_path parameter.")
        end
        ...
        selected_file = UI.select("Choose file:", xcstrings_files)
        selected_file
      end
      ...
      def self.select_target_language(param_language, available_languages, xcstrings_data)
        if param_language
          unless available_languages.include?(param_language)
            UI.user_error!("❌ Language '#{param_language}' not found in xcstrings file. Available: #{available_languages.join(', ')}")
          end
        ...
        language_options = available_languages.map do |lang_code|
          ...
          if Fastlane::Translate::DeepLLanguageMapper.supports_formality?(lang_code)
            display_name += " [supports formality]"
          end
          
          { display: display_name, code: lang_code, stats: stats }
        end
      ...
      def self.translate_language(xcstrings_data, xcstrings_path, source_language, target_language, formality, params)
        ...
        unless Fastlane::Translate::DeepLLanguageMapper.supported?(target_language)
          UI.user_error!("❌ Language '#{target_language}' is not supported by DeepL")
        end
      ...
      def self.extract_untranslated_strings(xcstrings_data, source_language, target_language, already_translated)
        ...
        xcstrings_data['strings'].each do |string_key, string_data|
          ...
          if localization['state'] == 'new' && (localization['value'].nil? || localization['value'].empty?)
      ...
      def self.handle_rate_limit_error(batch, index, total_batches)
      ...
      def self.handle_translation_error(error, batch, index, total_batches)
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(
            ...
            default_value: 20,
            verify_block: proc do |value|
              UI.user_error!("Batch size must be between 1 and 50") unless (1..50).include?(value)
            end
          ),
          FastlaneCore::ConfigItem.new(
            key: :free_api,
